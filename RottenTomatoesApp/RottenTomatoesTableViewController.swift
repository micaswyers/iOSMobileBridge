...
class RottenTomatoesTableViewController: UITableViewController {
                ...
                case 200:
                    if let downloadedData = data,
                        ...
                        movies = json["movies"] as? [NSDictionary] {
                            // working in a multi-threaded environment, so bounce this back to main queue. Ok.
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        if let moviesArray = self.moviesArray {
            ...
            let movieTitle = movie["title"] as? String ?? "Missing Title!"
            ...
            if let posters = movie["posters"] as? NSDictionary,
                posterURLString = posters["thumbnail"] as? String,
                posterURL = NSURL(string: posterURLString) {
                    cell.posterURL = posterURL
                    self.downloadImageURL(posterURL, forCell: cell)
            }
            cell.imageView?.image = nil // reset the image so that if you're on a slow connection, the recycling won't screw things up
        }
